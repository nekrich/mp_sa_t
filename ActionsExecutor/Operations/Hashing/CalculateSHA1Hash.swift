...
final class CalculateSHA1Hash: BaseFileOperation {
  override init(
    taskID: String,
    fileURL: URL,
  ...
  )
  {
    ...
    progress.fileOperationKind = .init("Calculating SHA1 hash")
  }
  ...
  override func main() {
    ...
    while autoreleasepool(invoking: { () -> Bool in
      ...
      data.withUnsafeBytes { (bytes) in
        _ = CC_SHA1_Update(&context, bytes.baseAddress, numericCast(bytes.count))
      }
    ...
    result = .success("SHA1: " + hashString)
